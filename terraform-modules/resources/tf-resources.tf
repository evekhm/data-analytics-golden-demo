...
####################################################################################
# Prior-Auth GKE Cluster
####################################################################################
# Subnet for prior-auth cluster
...
# Firewall rule for gke cluster
resource "google_compute_firewall" "cluster_subnet_firewall_rule" {
  project  = var.project_id
...
# Temp work bucket for priorauth cluster
# If you do not have a permanent temp bucket random ones will be created (which is messy since you do not know what they are being used for)
resource "google_storage_bucket" "prior_auth_bucket" {
  project                     = var.project_id
  name                        = "prior-auth-${var.storage_bucket}"
  location                    = var.region
  force_destroy               = true
  uniform_bucket_level_access = true
}
...
# Service account for GKE prior-auth cluster
resource "google_service_account" "gsa_priauth_service_account" {
  project      = var.project_id
  account_id   = "prior-auth-service-account"
  display_name = "Service Account for GKE Cluster Environment"
}


# Grant require worker role
resource "google_project_iam_member" "priorauth_service_account_worker_role" {
  project  = var.project_id
  role     = "roles/dataproc.worker"
  member   = "serviceAccount:${google_service_account.gsa_priauth_service_account.email}"

  depends_on = [
    google_service_account.gsa_priauth_service_account
  ]
}


# Grant editor (too high) to service account
resource "google_project_iam_member" "priorauth_service_account_editor_role" {
  project  = var.project_id
  role     = "roles/editor"
  member   = "serviceAccount:${google_service_account.gsa_priauth_service_account.email}"

  depends_on = [
    google_project_iam_member.priorauth_service_account_worker_role
  ]
}
...
# Create the cluster
# Prior-Auth Cluster
resource "google_container_cluster" "pa-cluster" {
  ...
  depends_on = [
    google_compute_subnetwork.cluster_subnet,
    google_storage_bucket.prior_auth_bucket,
    google_project_iam_member.priorauth_service_account_editor_role,
    google_compute_firewall.cluster_subnet_firewall_rule
  ]
}
